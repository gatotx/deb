...
# install v2ray
wget https://raw.githubusercontent.com/gatotx/deb/main/go.sh && chmod +x go.sh && ./go.sh
wget https://raw.githubusercontent.com/XTLS/Xray-install/main/install-release.sh && chmod +x install-release.sh && ./install-release.sh
...
iptables -I INPUT -m state --state NEW -m tcp -p tcp --dport 9083 -j ACCEPT
iptables -I INPUT -m state --state NEW -m tcp -p tcp --dport 9880 -j ACCEPT
...
iptables -I INPUT -m state --state NEW -m udp -p udp --dport 9083 -j ACCEPT
iptables -I INPUT -m state --state NEW -m udp -p udp --dport 9880 -j ACCEPT
iptables -I INPUT -m state --state NEW -m tcp -p tcp --dport 3646 -j ACCEPT
iptables -I INPUT -m state --state NEW -m udp -p udp --dport 3646 -j ACCEPT
...
systemctl start v2ray@tcp_xtls.service
systemctl restart trojan
systemctl enable trojan
systemctl restart v2ray
systemctl enable v2ray
chmod 755 /etc/v2ray/v2ray.key
cd
rm -f ins-vt.sh
